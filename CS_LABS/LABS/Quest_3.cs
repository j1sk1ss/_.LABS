...
{ public void Main()
    { Console.WriteLine("Choose a work number: ");
        new Quest3().GetType().GetMethod($"Work_{Math.ToInt(Console.ReadLine())}")?.
            Invoke(new Quest3(), null); }
    ...
    { Console.WriteLine("This work where u should type a line of text and program will calculate percentage of " +
            ...
            if (line == null) return;
                var letters = line.Where(char.IsLetter).Count();
                    Console.WriteLine($"The percentage of letters is " +
                                      $"{System.Math.Round(letters / (line.Length / 100.0))}%."); }
}