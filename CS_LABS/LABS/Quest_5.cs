...
{ private readonly Labs _labs = new Labs();
    ...
    { Console.WriteLine("Choose a work number: ");
        var q5 = new Quest5();
        var m = q5.GetType().GetMethod($"Work_{_labs.Math.ToInt(Console.ReadLine())}")?.
            Invoke(q5, null); }
    ...
    { return text.Split(" ").Aggregate("", (current, t) => current + (disct[t.ToLower()] + " "));
    }
    ...
    { Console.WriteLine("This work where u should type a word and program will show count of uniq letters: ");
        Console.WriteLine($"The count of uniq letters is: {Console.ReadLine()?.ToHashSet().Count}"); }
}